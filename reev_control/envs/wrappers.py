        ...
        if terminated or truncated:
            for key in self.info_keys:
                try:
                    ...
                    raise ValueError(f"InfoHistoryWrapper: Failed to convert history of key '{key}' to numpy array. Values: {self.info_history[key]}")
                
                if np.isnan(values).all():
                    info[f"{key}_sum"] = np.nan
                    info[f"{key}_avg"] = np.nan
                else:
                    info[f"{key}_sum"] = np.nansum(values)
                    info[f"{key}_avg"] = np.nanmean(values)
