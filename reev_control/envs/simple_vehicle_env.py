...
class SimpleVehicleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # print("="*20)
        # print(type(gen_power_grid), gen_power_grid.shape)
        # print(type(action['gen_power_grid']), action['gen_power_grid'].shape)
        # # print("gen_power_grid", gen_power_grid)
        ...
        info['action_norm'] = np.linalg.norm(
            action['gen_power_grid']) / action['gen_power_grid'].size
    ...
    def _compute_observation(self, simulated_states: dict):
        ...
        # check for nan values in final observation
        assert not np.isnan(obs).any(
        ), f"NAN observation detected for {self.trajectory_loader.file_name} for {[self._get_obs_names[idx] for idx in np.where(np.isnan(obs))]}"
