...
class InfoLogCallback(WandbCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        if self.info_buffer:
            rollout_means = {
                f"rollout/{key}_mean": np.nanmean(values)
...
class AdvantageLoggingCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> bool:
        ...
        if rb is not None and rb.advantages is not None:
            ...
            actions = rb.actions
            self.logger.record("test/action_mean_power_request", float(np.mean(actions[:,:,0])))
            self.logger.record("test/action_mean_engine_stop", float(np.mean(actions[:,:,1])))
            self.logger.record("test/action_std_power_request", float(np.std(actions[:,:,0])))
            self.logger.record("test/action_std_engine_stop", float(np.std(actions[:,:,1])))
